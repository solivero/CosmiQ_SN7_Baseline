...
for split in ['train', 'val']:
    aois = sorted([f for f in os.listdir(os.path.join(root_dir, split))
               if os.path.isdir(os.path.join(root_dir, split, f))])
    for i, aoi in enumerate(aois):
        print(i, "aoi:", aoi)
        aoi_dir = os.path.join(root_dir, split, aoi)
        im_dir = os.path.join(aoi_dir, 'images_masked/')
        json_dir = os.path.join(aoi_dir, 'labels_match/')
        out_dir_mask = os.path.join(aoi_dir, 'masks/')
        out_dir_mask_fbc = os.path.join(aoi_dir, 'masks_fbc/')
...
N_MONTHS = 3
for pop in pops:
    aois = sorted([f for f in os.listdir(os.path.join(root_dir, pop))
               if os.path.isdir(os.path.join(root_dir, pop, f))])
    for aoi in aois:
        aoi_dir = os.path.join(root_dir, pop, aoi)
        out_dir_cd_map = os.path.join(aoi_dir, 'change_maps/')
        mask_dir = os.path.join(aoi_dir, 'masks/')
        udm_dir = os.path.join(aoi_dir, 'UDM_masks/')
...
for pop in pops: 
    ...
    print(len(im1_list), len(im2_list), len(cm_list))
